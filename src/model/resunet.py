    ...
    def __init__(self, n_features_start=16, n_out=1, c0=True, ae_bin=False):
        ...
        if c0:
            ...
            self.encoder = nn.ModuleDict({
                ...
                # block 1
                'conv_block': ConvBlock(1, n_features_start), 'pool1': nn.MaxPool2d(pool_ks, pool_stride, pool_pad),
            ...
            self.encoder = nn.ModuleDict({
                ...
                # block 1
                'conv_block': ConvBlock(3, n_features_start), 'pool1': nn.MaxPool2d(pool_ks, pool_stride, pool_pad),
        ...
        self.decoder = nn.ModuleDict({# block 6
        ...
        if ae_bin:
            self.head = HeatmapAE_bin(n_features_start, n_out, kernel_size=1, stride=1, padding=0)
        else:
            self.head = Heatmap(n_features_start, n_out, kernel_size=1, stride=1, padding=0)

    def _forward_impl(self, x: Tensor) -> Tensor:
        downblocks = []
        for lbl, layer in self.encoder.items():
            x = layer(x)
            if 'block' in lbl: downblocks.append(
                x)  # NEXT loop is hon the values and so we don't hane the name as in the items of the previous loop
...
def _resunet(arch: str, n_features_start: int, n_out: int, c0: bool, #     block: Type[Union[BasicBlock, Bottleneck]],
        #     layers: List[int],
        pretrained: bool, progress: bool, ae_bin: bool, **kwargs, ) -> ResUnet:
...
def c_resunet(arch='c-ResUnet', n_features_start: int = 16, n_out: int = 1, c0=True, pretrained: bool = False,
              progress: bool = True, ae_bin: bool = False, **kwargs) -> ResUnet:
...
def load_model(resume_path, device, n_features_start=16, n_out=1, fine_tuning=False, unfreezed_layers=1, vae=False,
               ae_bin=False):
    if vae:
        model = nn.DataParallel(
            c_resunetVAE(arch='c-ResUnetVAE', n_features_start=n_features_start, n_out=n_out, device=device).to(device))

    else:
        model = nn.DataParallel(
            c_resunet(arch='c-ResUnet', n_features_start=n_features_start, n_out=n_out, ae_bin=ae_bin,
                    ...
                    for nc, cc in list(block[1].named_children())[::-1]:  # [1] because 0 is the name
                        if unfreezed_layers > 0:  # and isinstance(c, nn.Conv2d):
                            for n, p in cc.named_parameters():
                                p.requires_grad_(True)  # print(block, n, p.requires_grad)
                            print('unfreezed {}'.format(nc))
                        else:
                            for n, p in cc.named_parameters():
                                p.requires_grad_(False)  # print(block, n, p.requires_grad)
                        ...
                        if 'pool' not in nc:
                            if ix in ixs_list:
                                if unfreezed_layers > 0:
                                    ...
                                    for n, p in cc.named_parameters():
                                        p.requires_grad_(False)
                                    print('keep freezed {}'.format(nc))  # unfreezed_layers = int(unfreezed_layers) - 1
